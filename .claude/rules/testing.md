...
## CI Pipeline Rules
- **MANDATORY before every commit:** Run `npm run lint`, `npx tsc --noEmit` (in both `packages/server` and `packages/client`), and `npm test` locally. If any of these fail, fix the issue before committing. Do NOT push code that hasn't passed all three checks locally. This is non-negotiable — no exceptions.
